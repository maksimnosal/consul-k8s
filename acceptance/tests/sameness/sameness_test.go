...
func TestFailover_Connect(t *testing.T) {
	...
	ver, err := version.NewVersion("1.16.0")
	require.NoError(t, err)
	if cfg.ConsulVersion != nil && cfg.ConsulVersion.LessThan(ver) {
		t.Skipf("skipping this test because sameness is not supported in version %v", cfg.ConsulVersion.String())
	}

	// TODO (dans): left off here
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			staticClientPeerClusterContext := env.NthContext(t, 1)
