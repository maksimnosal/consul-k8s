...
func TestLoadTestCTGW(t *testing.T) {
	...
	env := os.Getenv("HCP_ENV")
	var err error
	if env != "" {
		apiHost, err = GetAPIAddr(env)
		require.NoError(t, err)
		authURL, err = GetAuthIDP(env)
		require.NoError(t, err)
		scadaAddr, err = GetScadaAddrWithoutProtocol(env)
		require.NoError(t, err)
	}
	...
	aclToken := os.Getenv("HCP_CONSUL_TOKEN")

	// This should never happen during the load test since we should have already controlled for it.
	if aclToken == "" {
		hcpCfg := hcp{
	...
	helmValues := map[string]string{
		...
		"global.acls.resources.requests.memory": "15Mi",
		"global.acls.resources.requests.cpu":    "5m",
		"global.acls.resources.limits.memory":   "15Mi",
		"global.acls.resources.limits.cpu":      "5m",

		"connectInject.initContainer.resources.requests.memory": "15Mi",
		"connectInject.initContainer.resources.requests.cpu":    "5m",

		"connectInject.initContainer.resources.limits.memory": "15Mi",
		"connectInject.initContainer.resources.limits.cpu":    "5m",
		...
		"telemetryCollector.resources.requests.cpu":        "100m",
		"telemetryCollector.resources.limits.cpu":          "100m",
	...
	consulCluster.ChartPath = "../../../../charts/consul"
	...
	nServices := 2
	nServicesStr := os.Getenv("LOAD_TEST_RUNS")
	if nServicesStr != "" {
		i, err := strconv.Atoi(nServicesStr)
		require.NoError(t, err)
		nServices = i
	}

	tmpl, err := template.ParseFiles("../../fixtures/bases/pingpong/template.tmpl")
	if err != nil {
		require.NoError(t, err)
	}

	for i := 0; i < nServices; i++ {
		data := &tmplData{Iteration: i, Replicas: 1}
		tmplIt, err := tmpl.Clone()
		require.NoError(t, err)
		...
		err = tmplIt.Execute(tempFile, data)
		require.NoError(t, err)
		// Close the temporary file to ensure that changes are saved
		err = tempFile.Close()
		require.NoError(t, err)
