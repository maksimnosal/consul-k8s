...
// Test that Connect works for an application with multiple ports via the workaround of registering each port as a
// service . The multiport application is a Pod listening on two ports. This tests inbound connections to each port
// of the multiport app, and outbound connections from the multiport app to static-server.
