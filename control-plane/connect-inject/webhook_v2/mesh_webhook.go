...
func (w *MeshWebhook) Handle(ctx context.Context, req admission.Request) admission.Response {
	...
	initContainer, err := w.containerInit(*ns, pod)
	if err != nil {
	...
	envoySidecar, err := w.consulDataplaneSidecar(*ns, pod)
	if err != nil {
...
func findServiceAccountVolumeMount(pod corev1.Pod) (corev1.VolumeMount, string, error) {
