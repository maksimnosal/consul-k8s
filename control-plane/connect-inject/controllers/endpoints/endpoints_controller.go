	...
	for _, nodeSvcs := range nodesWithSvcs {
		for _, svc := range nodeSvcs.Services {
			...
			if serviceDeregistered {
				err = r.deregisterNode(apiClient, k8sSvcName, k8sSvcNamespace, nodeSvcs.Node.Node)
				if err != nil {
...
func (r *Controller) deregisterNode(apiClient *api.Client, k8sServiceName, k8sServiceNamespace, nodeName string) error {
	...
	r.Log.Info("node services", "services", nodeServices)
	if len(nodeServices.Services) == 0 {
		r.Log.Info("deregistering node from consul", "node", nodeName, "services", nodeServices.Services)
